...
pb_12822_idx = 48
pa_18756_idx = 49
ne3_3869_idx = 50 #added by FH 3/4/25
hd_4102_idx = 51 #added by FH 6/27/25
ne3_3968_he_3970_idx = 52 #added by FH 6/27/25
he1_5875_idx = 53 #added by FH 6/27/25
pd_10052_idx = 54 #added by FH 11/10/25
fe2_12570_idx = 55 #added by FH 11/10/25
fe2_16440_idx = 56 #added by FH 11/10/25
...
pd_10052_broad_idx = t2_wave_idx + 40 #added by FH 11/10/25
fe2_12570_broad_idx = t2_wave_idx + 41 #added by FH 11/10/25
fe2_16440_broad_idx = t2_wave_idx + 42 #added by FH 11/10/25
...
## FH 2/24/25 - polynomial offset
def polynomial_offset(wave, coeffs, wave0):
    """Compute variable wavelength offset as a polynomial function."""
    return sum(c * (wave - wave0) ** i for i, c in enumerate(coeffs))

#FH modified 11/10/25 - expanded max. wavelength of fit
# and to account for redder end of spectrum 
def get_continuum_fit(pars, cont_model, x, first_node_index, nnodes, transition_wave, transition_wave2,  
                      fit_region, la_1216_obs, polycont, lincont):
    ...
    # split the spline model into blue wavelengths (blue), 
    # middle wavelengths (mid) and red wavelengths (red) thirds.
    ...
    if len(spline_x_values_red) > 0:
        ...
        w = np.where((x >= transition_wave2) & (x < max_wave  + fit_region))
...
def emissionline_model(pars, x, comps, polycont, lincont):
    ...
    if comps == False:
        ...
        #FH 11/10/25
        pd_10052_amp = pars[pd_10052_idx] #added by FH 11/10/25
        fe2_12570_amp = pars[fe2_12570_idx] #added by FH 11/10/25
        fe2_16440_amp = pars[fe2_16440_idx] #added by FH 11/10/25
        h2_19570_amp = pars[h2_19570_idx] #added by FH 11/10/25
        ...
        si7_24833_obs = si7_24833_vac * (1 + z) #added by FH 2/28/25
        ne3_3869_obs = ne3_3869_vac * (1 + z) #added by FH 3/4/25
        ...
        # FH 11/10/25:
        add_emission_line_to_model(line_model, pd_10052_obs, pd_10052_amp)
        add_emission_line_to_model(line_model, fe2_12570_obs, fe2_12570_amp)
        add_emission_line_to_model(line_model, fe2_16440_obs, fe2_16440_amp)
        add_emission_line_to_model(line_model, h2_19570_obs, h2_19570_amp)
    ...
    if comps == True:
        ...
        # si7_24833_obs = si7_24833_vac * (1 + z) #added by FH 2/28/25
        # bra_40523_obs = bra_40523_vac * (1 + z) #added by FH 11/10/25
        ...
        # FH 11/10/25:
        pd_10052_amp = pars[pd_10052_idx];          pd_10052_broad_amp = pars[pd_10052_broad_idx]
        fe2_12570_amp = pars[fe2_12570_idx];        fe2_12570_broad_amp = pars[fe2_12570_broad_idx]
        fe2_16440_amp = pars[fe2_16440_idx];        fe2_16440_broad_amp = pars[fe2_16440_broad_idx]
        h2_19570_amp = pars[h2_19570_idx];        h2_19570_broad_amp = pars[h2_19570_broad_idx]
        ...
        cont_model_double_gauss = get_continuum_fit(pars, cont_model_double_gauss, x, first_node_index, nnodes, transition_wave, transition_wave2, fit_region, la_1216_obs, polycont, lincont )
        ...
        # FH 11/10/25:
        add_emission_line_to_model2gauss(line_model_narrow_gauss, pd_10052_obs, pd_10052_amp, pd_10052_broad_amp)
        add_emission_line_to_model2gauss(line_model_narrow_gauss, fe2_12570_obs, fe2_12570_amp, fe2_12570_broad_amp)
        add_emission_line_to_model2gauss(line_model_narrow_gauss, fe2_16440_obs, fe2_16440_amp, fe2_16440_broad_amp)
        add_emission_line_to_model2gauss(line_model_narrow_gauss, h2_19570_obs, h2_19570_amp, h2_19570_broad_amp)
        add_emission_line_to_model2gauss(line_model_narrow_gauss, h2_21218_obs, h2_21218_amp, h2_21218_broad_amp)
...
### FH started 2/18/25:
## Residual function for Combined spectra
def model_resid_comb(pars, fjac=None, lam1=None, flux1=None, err1=None, lam2=None, flux2=None, err2=None, comps=False, polycont=False, lincont=False, degree=2):  
    # amp, center, fwhm1, fwhm2, offset = pars  
    
    # wave0 = np.mean(lam1)  # Reference wavelength for polynomial
    # offset_coeffs = pars[4:4 + degree + 1]  # Polynomial coefficients for wavelength offset
    # # lam2_shifted = lam2 + polynomial_offset(lam2, offset_coeffs, wave0)
    
    model1 = emissionline_model(pars, lam1, comps, polycont, lincont)
    model2 = emissionline_model(pars, lam2, comps, polycont, lincont)

    resid1 = (flux1 - model1) / err1  
    resid2 = (flux2 - model2) / err2

    status = 0

    return [status, np.concatenate([resid1, resid2])]


## FH updated 11/10/25
## FH 2/4/25 - this is the version for POPPIES:
def fit_obj(input_list,filter='F444W'):
    ''' This version is for both cwt and full object list'''
    print('\nRunning fit_obj...\n')
    ...
    #based on filter:
    transition_wave1 = config_pars['lambda_min_{}'.format(filter)]
    transition_wave2 = config_pars['lambda_max_{}'.format(filter)]
    ...
    si7_24833_obs = si7_24833_vac * (1 + z_in) #added by FH 2/27/25

    ne3_3869_obs = ne3_3869_vac * (1 + z_in) #added by FH 3/4/25
    ...
    pd_10052_obs = pd_10052_vac * (1 + z_in)
    fe2_12570_obs = fe2_12570_vac * (1 + z_in)
    fe2_16440_obs = fe2_16440_vac * (1 + z_in)
    h2_19570_obs = h2_19570_vac * (1 + z_in)
    h2_21218_obs = h2_21218_vac * (1 + z_in)
    ...
    w = np.where((lam_spec < la_1216_obs - fit_region) | (lam_spec > max_wave + fit_region))
    mask_spec[w] = 1.0
    ...
    mask_emission_lines(pg_10941_obs, pg_10941_obs)  # FH added 2/20/25
    ...
    mask_emission_lines(pd_10052_obs, pd_10052_obs) # FH added 11/10/25
    mask_emission_lines(fe2_12570_obs, fe2_12570_obs) # FH added 11/10/25
    mask_emission_lines(fe2_16440_obs, fe2_16440_obs) # FH added 11/10/25
    mask_emission_lines(h2_19570_obs, h2_19570_obs) # FH added 11/10/25
    ...
    if out.status > 0:
        ...
        # w = np.where((lam_spec > (la_1216_obs - fit_region)) & (lam_spec < (bra_40523_obs + fit_region)))
        ...
        ## added by FH 11/10/25:
        pd_10052_obs = pd_10052_vac * (1 + z_out)
        fe2_12570_obs = fe2_12570_vac * (1 + z_out)
        fe2_16440_obs = fe2_16440_vac * (1 + z_out)
        h2_19570_obs = h2_19570_vac * (1 + z_out)
        h2_21218_obs = h2_21218_vac * (1 + z_out)
        ...
        if comps_in == False:
            ...
            ## FH added 11/10/25:
            pd_10052_flux, pd_10052_err, pd_10052_ew_obs = calculate_emission_line_flux(
                pd_10052_obs, pd_10052_idx, pd_10052_vac, False, polycont_in, lincont_in)            

            fe2_12570_flux, fe2_12570_err, fe2_12570_ew_obs = calculate_emission_line_flux(
                fe2_12570_obs, fe2_12570_idx, fe2_12570_vac, False, polycont_in, lincont_in)            

            fe2_16440_flux, fe2_16440_err, fe2_16440_ew_obs = calculate_emission_line_flux(
                fe2_16440_obs, fe2_16440_idx, fe2_16440_vac, False, polycont_in, lincont_in)            

            h2_19570_flux, h2_19570_err, h2_19570_ew_obs = calculate_emission_line_flux(
                h2_19570_obs, h2_19570_idx, h2_19570_vac, False, polycont_in, lincont_in)            

            h2_21218_flux, h2_21218_err, h2_21218_ew_obs = calculate_emission_line_flux(
                h2_21218_obs, h2_21218_idx, h2_21218_vac, False, polycont_in, lincont_in)            

            pah_32890_flux, pah_32890_err, pah_32890_ew_obs = calculate_emission_line_flux(
                pah_32890_obs, pah_32890_idx, pah_32890_vac, False, polycont_in, lincont_in)            
        ...
        if comps_in == True:
            ...
            ## FH added 11/10/25:
            pd_10052tot_flux, pd_10052tot_err, pd_10052tot_ew_obs, pd_10052nar_flux, pd_10052nar_err, pd_10052nar_ew_obs, pd_10052bro_flux, pd_10052bro_err, pd_10052bro_ew_obs = calculate_emission_line_flux(
                pd_10052_obs, pd_10052_idx, pd_10052_vac, pd_10052_broad_idx, True, polycont_in, lincont_in)            

            fe2_12570tot_flux, fe2_12570tot_err, fe2_12570tot_ew_obs, fe2_12570nar_flux, fe2_12570nar_err, fe2_12570nar_ew_obs, fe2_12570bro_flux, fe2_12570bro_err, fe2_12570bro_ew_obs = calculate_emission_line_flux(
                fe2_12570_obs, fe2_12570_idx, fe2_12570_vac, fe2_12570_broad_idx, True, polycont_in, lincont_in)            

            fe2_16440tot_flux, fe2_16440tot_err, fe2_16440tot_ew_obs, fe2_16440nar_flux, fe2_16440nar_err, fe2_16440nar_ew_obs, fe2_16440bro_flux, fe2_16440bro_err, fe2_16440bro_ew_obs = calculate_emission_line_flux(
                fe2_16440_obs, fe2_16440_idx, fe2_16440_vac, fe2_16440_broad_idx, True, polycont_in, lincont_in)            

            h2_19570tot_flux, h2_19570tot_err, h2_19570tot_ew_obs, h2_19570nar_flux, h2_19570nar_err, h2_19570nar_ew_obs, h2_19570bro_flux, h2_19570bro_err, h2_19570bro_ew_obs = calculate_emission_line_flux(
                h2_19570_obs, h2_19570_idx, h2_19570_vac, h2_19570_broad_idx, True, polycont_in, lincont_in)            
          
            h2_21218tot_flux, h2_21218tot_err, h2_21218tot_ew_obs, h2_21218nar_flux, h2_21218nar_err, h2_21218nar_ew_obs, h2_21218bro_flux, h2_21218bro_err, h2_21218bro_ew_obs = calculate_emission_line_flux(
                h2_21218_obs, h2_21218_idx, h2_21218_vac, h2_21218_broad_idx, True, polycont_in, lincont_in)            
          
            pah_32890tot_flux, pah_32890tot_err, pah_32890tot_ew_obs, pah_32890nar_flux, pah_32890nar_err, pah_32890nar_ew_obs, pah_32890bro_flux, pah_32890bro_err, pah_32890bro_ew_obs = calculate_emission_line_flux(
                pah_32890_obs, pah_32890_idx, pah_32890_vac, pah_32890_broad_idx, True, polycont_in, lincont_in)            
...
## FH updated 11/10/25
## FH 2/19/25 - Combined version, with R+C spectra
def fit_obj_comb(input_list,filter='F444W'):
    ''' This version is for both cwt and full object list'''
    print('\nRunning fit_obj_comb...\n')

    ## FH updated these 2/20/25

    #Extract inputs from spectra:
    lam_spec    = input_list[0]  # for R spec
    flux_spec   = input_list[1]  # for R spec
    error_spec  = input_list[2]  # for R spec
    lam_spec2    = input_list[3]  # for C spec
    flux_spec2   = input_list[4]  # for C spec
    error_spec2  = input_list[5]  # for C spec
    config_pars = input_list[6]
    z_in        = input_list[7]  # the best-guess redshift or the current line-list redshift
    fwhm_guess  = input_list[8]  # red fwhm guess
    beam_name   = input_list[9]  # object id.
    ab_image_max= input_list[10]  # MDR 2022/06/30
    fast_fit    = input_list[11]  # MDR 2022/06/30
    comps_in    = input_list[12]  # KVN 01-Aug-2024
    polycont_in = input_list[13] # KVN 05-Aug-2024
    lincont_in  = input_list[14] # KVN 06-Aug-2024
    ...
    flux_spec2  = (flux_spec2  / scl)
    error_spec2 = (error_spec2 / scl)
    ...
    pa_18756_obs = pa_18756_vac * (1 + z_in)
    si7_24833_obs = si7_24833_vac * (1 + z_in) #added by FH 2/27/25
    bra_40523_obs = bra_40523_vac * (1 + z_in) #added by FH 11/10/25
    ...
    # FH updated 2/28/25: First set the mask equal to zero everywhere and mask regions outside the bluest line.
    mask_spec = (lam_spec * 0.0)
    ...
    mask_spec2 = (lam_spec2 * 0.0)
    # w2 = np.where((lam_spec2 < la_1216_obs - fit_region) | (lam_spec2 > bra_40523_obs + fit_region))
    w2 = np.where((lam_spec2 < la_1216_obs - fit_region) | (lam_spec2 > max_wave + fit_region))
    mask_spec2[w2] = 1.0

    ## FH modified 2/20/25:
    def mask_emission_lines(blue_line, red_line, lam=lam_spec, masked=mask_spec):
        w = np.where((lam > (blue_line - line_mask)) & (lam < (red_line + line_mask)))
        masked[w] = 1.0
    ...
    wave = lam_spec
    mask_emission_lines(la_1216_obs, la_1216_obs, wave, mask_spec)
    mask_emission_lines(n5_1238_obs, n5_1242_obs, wave, mask_spec)
    mask_emission_lines(c4_1548_obs, c4_1550_obs, wave, mask_spec)
    mask_emission_lines(h2_1640_obs, h2_1640_obs, wave, mask_spec)
    mask_emission_lines(o3_1660_obs, o3_1666_obs, wave, mask_spec)
    mask_emission_lines(s3_1883_obs, s3_1892_obs, wave, mask_spec)
    mask_emission_lines(c3_1907_obs, c3_1909_obs, wave, mask_spec)
    mask_emission_lines(m2_2796_obs, m2_2803_obs, wave, mask_spec)
    mask_emission_lines(o2_3727_obs, o2_3730_obs, wave, mask_spec)
    mask_emission_lines(hg_4342_obs, o3_4363_obs, wave, mask_spec)
    mask_emission_lines(h2_4686_obs, h2_4686_obs, wave, mask_spec)
    mask_emission_lines(hb_4863_obs, o3_5007_obs, wave, mask_spec)
    mask_emission_lines(o1_6300_obs, o1_6363_obs, wave, mask_spec)
    mask_emission_lines(n2_6550_obs, s2_6731_obs, wave, mask_spec)
    mask_emission_lines(s3_9069_obs, s3_9069_obs, wave, mask_spec)
    mask_emission_lines(s3_9532_obs, s3_9532_obs, wave, mask_spec)
    mask_emission_lines(he10830_obs, he10830_obs, wave, mask_spec)
    mask_emission_lines(pg_10941_obs, pg_10941_obs, wave, mask_spec)
    mask_emission_lines(pb_12822_obs, pb_12822_obs, wave, mask_spec)
    mask_emission_lines(pa_18756_obs, pa_18756_obs, wave, mask_spec)
    # mask_emission_lines(si7_24833_obs, si7_24833_obs) # FH added 2/28/25
    mask_emission_lines(bra_40523_obs, bra_40523_obs) # FH added 11/10/25
    ...
    # ## then, for another spectrum:
    # wave = lam_spec2
    # mask_emission_lines(la_1216_obs, la_1216_obs, wave, mask_spec2)
    # mask_emission_lines(n5_1238_obs, n5_1242_obs, wave, mask_spec2)
    # mask_emission_lines(c4_1548_obs, c4_1550_obs, wave, mask_spec2)
    # mask_emission_lines(h2_1640_obs, h2_1640_obs, wave, mask_spec2)
    # mask_emission_lines(o3_1660_obs, o3_1666_obs, wave, mask_spec2)
    # mask_emission_lines(s3_1883_obs, s3_1892_obs, wave, mask_spec2)
    # mask_emission_lines(c3_1907_obs, c3_1909_obs, wave, mask_spec2)
    # mask_emission_lines(m2_2796_obs, m2_2803_obs, wave, mask_spec2)
    # mask_emission_lines(o2_3727_obs, o2_3730_obs, wave, mask_spec2)
    # mask_emission_lines(hg_4342_obs, o3_4363_obs, wave, mask_spec2)
    # mask_emission_lines(h2_4686_obs, h2_4686_obs, wave, mask_spec2)
    # mask_emission_lines(hb_4863_obs, o3_5007_obs, wave, mask_spec2)
    # mask_emission_lines(o1_6300_obs, o1_6363_obs, wave, mask_spec2)
    # mask_emission_lines(n2_6550_obs, s2_6731_obs, wave, mask_spec2)
    # mask_emission_lines(s3_9069_obs, s3_9069_obs, wave, mask_spec2)
    # mask_emission_lines(s3_9532_obs, s3_9532_obs, wave, mask_spec2)
    # mask_emission_lines(he10830_obs, he10830_obs, wave, mask_spec2)
    # mask_emission_lines(pg_10941_obs, pg_10941_obs, wave, mask_spec)
    # mask_emission_lines(pb_12822_obs, pb_12822_obs, wave, mask_spec2)
    # mask_emission_lines(pa_18756_obs, pa_18756_obs, wave, mask_spec2)

    # w2 = np.where(mask_spec2 == 0.0)[0]

    # cont_guess2 = np.median(flux_spec2[w2])
    ...
    # pguess[4:4 + degree + 1] = 0.0  # Offset polynomial coeffs
    ...
    # initialize the wavelength, flux, and error function arguments.
    fa_cont = {'lam1':lam_spec[w], 'flux1':flux_spec[w], 'err1':error_spec[w],
               'lam2':lam_spec2[w2], 'flux2':flux_spec2[w2], 'err2':error_spec2[w2]} # FH 2/20/25
    # fa_cont = {'lam1': lam_spec[w], 'flux1': flux_spec[w], 'err1': error_spec[w],
    #     'lam2': lam_spec2[w2], 'flux2': flux_spec2[w2], 'err2': error_spec2[w2],
    #     'comps': comps_in, 'polycont': polycont_in, 'lincont': lincont_in, 'degree': degree}
    
    # # call mpfit for the continuum-only fit model.
    out = mpfit(model_resid_comb, pguess_cont, functkw = fa_cont, parinfo = parinfo_cont, quiet = True)
    ...
    if out.status > 0:
        ...
        # FH modified 2/20/25
        def estimate_emission_line_amplitudes(centroid,lam=lam_spec,flux=flux_spec):
            if ((centroid > np.min(lam)) & (centroid < np.max(lam))):
                if (centroid < transition_wave1): disp = disp_blue
                if (centroid > transition_wave1): disp = disp_red
                wline = np.where((lam > (centroid - 5.0 * disp_blue)) & (lam < (centroid + 5.0 * disp)))
                if (np.size(wline) > 0):
                    peakval = np.max(flux[wline])
                    amplitude_guess = (peakval - emissionline_model(out.params, np.array([centroid]), comps_in, polycont_in, lincont_in))[0] 
                    # Peak minus continuum.
                    if (amplitude_guess < 0.0):
                        amplitude_guess = 0.0
        ...
        #### KVN: Otherwise, set lower aplitude limit to zero 
            ...
            parinfo[n2_6585_broad_idx]['value'] = (estimate_emission_line_amplitudes(n2_6550_obs) / 3.0) # modified by FH 2/25/25
        ...
        # define the wavelength range to fit (FH modified 2/28/25)
        ## FH updated 11/17/25:
        max_wave = 60000
        ...
        # initialize the wavelength, flux, and error function arguments.
        fa = {'lam1':lam_spec[w], 'flux1':flux_spec[w], 'err1':error_spec[w],
            'lam2':lam_spec2[w2], 'flux2':flux_spec2[w2], 'err2':error_spec2[w2]} # FH 2/20/25
        ...
        out = mpfit(model_resid_comb, pguess, functkw = fa, parinfo = parinfo, quiet = True, maxiter=100)
...
## FH 2/27/25
def continuum_model(pars, x, comps, polycont, lincont):
    # The emission line model includes 'pars' for the continuum and emission line
    # parameters. The input 'x' is the wavelength array, which can include gaps.
    # mpfit.py allows for 'pegged', 'fixed', and 'limited' parameters with 'limits'.
    # See the mpfit.py code for a detailed description, but it is useful to note
    # that 'pegged' parameters are simply those 'fixed' at their upper or lower limit.
    ''' JUST continuum model '''
    # fixed parameters
    nnodes           = int(pars[nnodes_idx]) # calculated from len(node_wave) and passed here.
    fit_region       = pars[fitreg_idx]
    transition_wave  = pars[t_wave_idx]
    ...
    if comps == False:
        ...
        # initialize the continuum model as list of zeros.
        cont_model = x * 0.
...
# ########## COMBINED EMISSION LINE MODEL 

# ### FH combined emission model - 2/24/25
# #pars, x, comps, polycont, lincont
# def emissionline_model_comb(pars, lam1, lam2, comps, polycont, lincont, degree=2):
#     # The emission line model includes 'pars' for the continuum and emission line
#     # parameters. The inputs 'lam1', 'lam2' the wavelength arrays, which can include gaps.
#     # mpfit.py allows for 'pegged', 'fixed', and 'limited' parameters with 'limits'.
#     # See the mpfit.py code for a detailed description, but it is useful to note
#     # that 'pegged' parameters are simply those 'fixed' at their upper or lower limit.
...
#     ################### THIS IS WHERE FITTING WILL OCCUR ON TWO SPECTRA

#     '''
#     The line_model that will be fit to the data consists of a flux continuum
#     and emission lines. The emission lines are added to the model one at a time
#     using the add_emission_line_to_model() function defined below. The function
#     extracts a wavelength array     ed on the line with a width controlled by
#     the 'fit_region' parameter defined in the 'default.config' file. There are
#     several groups of emission lines such as [O III]+HB, and HA+[N II]+[S II]
#     that have special constraints and are added using specialized code below.
#     Closely-spaced doublet lines are added with the add_doublet_lines_to_model()
...
#     ######## Carry out fit without double gaussians
#     ######## ---> this requires fewer fit parameters, but all need to be initialized. 
#     ######## ---> Later hin the code, unused params get fixed to zero
#     if comps == False:
#         # define the emission line amplitude model parameters.
#         # those set as ratios are defined relative to the other line in the doublet.
#         # the physically allowable range for each ratio is set later in the code.
...
#         model1 =  cont_model_double_gauss + line_model_narrow_gauss #+ line_model_broad_gauss
...
#         model2 =  cont_model_double_gauss + line_model_narrow_gauss #+ line_model_broad_gauss
