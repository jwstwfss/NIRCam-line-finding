...
def initialize_arrays(data, bluecut, redcut):
    ...
    lam = data['wave'][3:-3]
    flux = data['flux'][3:-3]
    error = data['error'][3:-3]
    contam = data['contam'][3:-3]
    zeros = data['zeroth'][3:-3]
...
def check_grism(tbdata, min_wav, max_wav):
    ...
    if tbdata is not None:
        ...
        # print('tbdata is not None',_specdata)
...
### FH 12/23/24 POPPIES version:
def trim_spec_1filter(tbdata_blue, config_pars, filter, mask_zeros=False, return_masks=False):
    """Create one array of spectra, etc. from ONE filter
    
    If masking: 
        don't need to mask wavelength or zeroth orders, keep these for 
        writing out file
    ...
    ### only fit finite data
    # lam_spec_blue, flux_spec_blue, error_spec_blue, contam_spec_blue, zero_spec_blue = check_grism(tbdata_blue, config_pars['lambda_min_{}'.format(filter)], config_pars['lambda_max_{}'.format(filter)])
    ...
    error_spec = np.array(error_spec_blue)
    contam_spec = np.array(contam_spec_blue)
    zero_spec = np.array(zero_spec_blue)
...
def trim_spec(tbdata_blue,  tbdata_green, tbdata_red, config_pars, mask_zeros=False, return_masks=False):
    """Create one array of spectra, etc. from multiple filters
    
    If masking: 
        don't need to mask wavelenghth or zeroth orders, keep these for 
        writing out file
...
def trim_spec_orig(tbdata_blue, tbdata_red, config_pars, mask_zeros=False, return_masks=False):
...
def trim_spec_OLD(tbdata_blue, tbdata_green, tbdata_red, config_pars, mask_zeros=False, return_masks=False):
    """KVN version -- Create one array of spectra, etc. from multiple grism 1d files
